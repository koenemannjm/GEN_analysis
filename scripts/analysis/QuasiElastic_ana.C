...
int QuasiElastic_ana(const char *configfilename, std::string filebase="outfiles/QE_test")
{
  ...
  int ngrinch_hits; setrootvar::setbranch(C,"Ndata.bb.grinch_tdc.hit","time",&ngrinch_hits);  
  ...
  TString outFile = Form("%s_GEN%d_sbs%dp_nucleon_%s_model%d_data.root", 
  ...
  while (C->GetEntry(nevent++)) {
    ...
    coinCut = coin_time > 3.5 && coin_time < 16.3;
