...
int runs[nruns] = {2085};
TString titles[nruns] = {"45 uA on He3"};
...
double Danning_CM_online(TH1F *hAPV, int fiber, int adc_ch, int isamp, int niter, int flag){
  ...
  for(int iter = 0; iter < niter; iter++){
    ...
    for(int istrip = 0; istrip < 128; istrip++){
      if(iter == 0){
	cm_min = CMmean[fiber][adc_ch] - 5*CMrms[fiber][adc_ch];
	cm_max = CMmean[fiber][adc_ch] + 5*CMrms[fiber][adc_ch];
      }
      else {
	// Flag == 1 for extra sqrt(6) factor
	if(flag == 1 ) cm_min = cm_temp - fZeroSuppressRMS*fRMS_ConversionFactor*PedRMS[fiber][adc_ch][istrip];
	else cm_min = cm_temp - fZeroSuppressRMS*PedRMS[fiber][adc_ch][istrip];
	
	if(flag == 1 ) cm_max = cm_temp + fZeroSuppressRMS*fRMS_ConversionFactor*PedRMS[fiber][adc_ch][istrip];
	else cm_max = cm_temp + fZeroSuppressRMS*fRMS_ConversionFactor*PedRMS[fiber][adc_ch][istrip];
      }
            
      double adc = hAPV->GetBinContent(istrip + 129*isamp);
      if(adc > cm_min && adc < cm_max){
	ADCsum += adc;
	n_keep++;
      }
    }
    
    cm_temp = ADCsum/n_keep;
...
double Histogramming_CM(double strip_ADC[][fN_MPD_TIME_SAMP], int fiber, int adc_ch, int isamp){
  
  double cm_mean = CMmean[fiber][adc_ch];
  double cm_rms = CMrms[fiber][adc_ch];
  
  if( fNeventsRollingAverage_by_APV[fiber][adc_ch] >= std::min(100, fN_MPD_TIME_SAMP*fNeventsCommonModeLookBack ) ){
    cm_mean = fCommonModeRollingAverage_by_APV[fiber][adc_ch];
    cm_rms = fCommonModeRollingRMS_by_APV[fiber][adc_ch];
  }
    ...
    for( int ihit=0; ihit<128; ihit++ ){
      double ADC = strip_ADC[ihit][isamp];
...
void process_run(int irun, int run,bool event_display,TH2F *hCM_Danning_2iter, TH2F *hCM_Danning_no_factor, TH2F *hCM_Danning, TH2F *hCM_Histo){



  if(event_display) gROOT->SetBatch(kTRUE);
  ...
  TString Rootfiles = "/lustre19/expphy/volatile/halla/sbs/jeffas/GEN_root/Rootfiles/";

  TString Rootfile = Form(Rootfiles + "hit_0_e1209016_%i.root",run);
  ...
  //TH2F *hCM_online_2iter = new TH2F(titles[irun],"2 Iterations Danning - Sorting;APV;ADC",15,0,15,500,-100,100);
  //TH2F *hCM_online_3iter = new TH2F(titles[irun],"3 Iterations Danning - Sorting;APV;ADC",15,0,15,500,-100,100);
  ...
  for(int i=0; i < 15; i++){
    hAPV_pedsub[i] = new TH1F(Form("hAPV_pedsub_%i",i),Form("APV %i;Strip;ADC",i),768,0,768);
  ...
  while(t->GetEntry(ievent++) && ievent < 5000){
	...
	for(int isamp=0; isamp < 6; isamp++){
	  ...
	  hAPV_pedsub[adc_ch[ich]]->SetBinContent(real_strip + 129*isamp, adc_all[isamp][ich]);
	}
      }
    }
	...
	double CM_Danning = Danning_CM_online(hAPV_pedsub[i],fMPD,i,isamp,3,1);
	...
	hCM_Danning_2iter->Fill(i,Danning_CM_online(hAPV_pedsub[i],fMPD,i,isamp,2,1) - CM_Sorting);
	hCM_Danning_no_factor->Fill(i,Danning_CM_online(hAPV_pedsub[i],fMPD,i,isamp,2,0) - CM_Sorting);
	
	//hCM_Histo->Fill(i,Histogramming_CM(strip_ADC[fMPD][i],fMPD, i, isamp) - CM_Sorting);
	hCM_Danning->Fill(i,Danning_CM_online(hAPV_pedsub[i],fMPD,i,isamp,3,1) - CM_Sorting);	    
    ...
    if((ievent - start_event) % 101 == 0 && event_display){
      //if(ievent > start_event && event_display){
      cout<<ievent<<endl;
	...
	//l_Histo[isamp] = new TLine(128*isamp, Histogramming_CM(strip_ADC[fMPD][iapv],fMPD, iapv, isamp), 128*(isamp+1), Histogramming_CM(strip_ADC[fMPD][iapv],fMPD,iapv,isamp));
	//l_Histo[isamp]->SetLineColor(kOrange);
	//l_Histo[isamp]->Draw("same");
      ...
      //leg->AddEntry(l_Histo[0],"Histogramming CM","l");
      if(idisplay == 0) leg->Draw("same");
    ...
    //if(ievent >130 && event_display && idisplay < ndisplay){
    if((ievent - start_event) % 101 == 0 && event_display && idisplay < ndisplay){
    //if(ievent > start_event && event_display && idisplay < ndisplay){
      cout<<ievent<<endl;
      idisplay++;
  ...
  c4->cd(1);
  hCM_Danning_no_factor->Draw("colz");

  c4->cd(2);
  hCM_Danning_2iter->Draw("colz");
  //htest->Draw("colz");
  ...
  hCM_Danning->Draw("colz");

  if(irun == 0) c4->Print("../plots/CM_Method_Comparison.pdf(");
  else c4->Print("../plots/CM_Method_Comparison.pdf");
...
void CM_event_display(bool event_display = false, bool flipped_events = false){

  TString output = "../plots/neg_pulse_study.pdf";
  
  int ped_run = 1899;

  TString DB = "/w/halla-scshelf2102/sbs/jeffas/SBS_OFFLINE/SBS-replay/DB/gemped/";

  TString Pedfile = Form(DB + "daq_ped_bb_gem_run%i.dat",ped_run);
  TString CMfile = Form(DB + "daq_cmr_bb_gem_run%i.dat",ped_run); 
  ...
  TH2F *hCM_Danning[nruns];
  TH2F *hCM_Histo[nruns];
  TH2F *hCM_Danning_2iter[nruns];
  TH2F *hCM_Danning_no_factor[nruns];
  ...
  for( int irun = 0; irun < nruns; irun++){
    ...
    hCM_Danning[irun] = new TH2F(titles[irun],"3 Iterations, Extra Factor, Danning - Sorting CM;APV;ADC",15,0,15,500,-100,100);    
    hCM_Danning_2iter[irun] = new TH2F(titles[irun],"2 Iterations, Extra Factor, Danning - Sorting CM;APV;ADC",15,0,15,500,-100,100);    
    hCM_Danning_no_factor[irun] = new TH2F(titles[irun],"2 Iterations, No Factor, Danning - Sorting CM;APV;ADC",15,0,15,500,-100,100);    
    hCM_Histo[irun] = new TH2F(titles[irun],"Histogramming - Sorting CM;APV;ADC",15,0,15,500,-100,100);    
    ...
    process_run(irun, runs[irun],event_display,hCM_Danning_2iter[irun], hCM_Danning_no_factor[irun], hCM_Danning[irun], hCM_Histo[irun]);
  ...
  double y_CM_Danning[nruns][nadc], y_CM_Histo[nruns][nadc];
  double y_CM_Danning_err[nruns][nadc], y_CM_Histo_err[nruns][nadc];
  double y_CM_Danning_2iter[nruns][nadc], y_CM_Danning_2iter_err[nruns][nadc];
  double y_CM_Danning_no_factor[nruns][nadc], y_CM_Danning_no_factor_err[nruns][nadc];

  for(int iapv=0; iapv < nadc-1; iapv++) x_APV[iapv] = iapv + 1;

  TCanvas *c3 = new TCanvas("c3","",1400,1200);
  c3->Divide(2,2);

  TGraphErrors *g_CM_Danning_2iter[nruns];
  TGraphErrors *g_CM_Danning_no_factor[nruns];

  TGraphErrors *g_CM_Danning[nruns];
  TGraphErrors *g_CM_Histo[nruns];
  ...
  for(int irun = 0; irun < nruns; irun++){
    ...
    for(int iapv=0; iapv < nadc; iapv++){
      ...
      hCM_Danning[irun]->ProjectionY("",x_APV[iapv],x_APV[iapv])->Fit("fit","q0");
 
      y_CM_Danning[irun][iapv] = fit->GetParameter(1);
      y_CM_Danning_err[irun][iapv] = fit->GetParameter(2);
      
      hCM_Histo[irun]->ProjectionY("",x_APV[iapv],x_APV[iapv])->Fit("fit","q0");

      y_CM_Histo[irun][iapv] = fit->GetParameter(1);
      y_CM_Histo_err[irun][iapv] = fit->GetParameter(2);

      hCM_Danning_2iter[irun]->ProjectionY("",x_APV[iapv],x_APV[iapv])->Fit("fit","q0");
 
      y_CM_Danning_2iter[irun][iapv] = fit->GetParameter(1);
      y_CM_Danning_2iter_err[irun][iapv] = fit->GetParameter(2);

      hCM_Danning_no_factor[irun]->ProjectionY("",x_APV[iapv],x_APV[iapv])->Fit("fit","q0");
      
      y_CM_Danning_no_factor[irun][iapv] = fit->GetParameter(1);
      y_CM_Danning_no_factor_err[irun][iapv] = fit->GetParameter(2);
    }
    
    g_CM_Danning_2iter[irun] = new TGraphErrors(nadc-1,x_APV,y_CM_Danning_2iter[irun],0,y_CM_Danning_2iter_err[irun]);
    g_CM_Danning_2iter[irun]->SetTitle("2 Iterations, With Factor, Danning - Sortin;APV;ADC");
    g_CM_Danning_2iter[irun]->SetMarkerStyle(8);
    g_CM_Danning_2iter[irun]->SetMarkerColor(icolor);
    g_CM_Danning_2iter[irun]->SetLineColor(icolor);
    legend->AddEntry(g_CM_Danning_2iter[irun],titles[irun],"l");
    
    g_CM_Danning_no_factor[irun] = new TGraphErrors(nadc-1,x_APV,y_CM_Danning_no_factor[irun],0,y_CM_Danning_no_factor_err[irun]);
    g_CM_Danning_no_factor[irun]->SetTitle("2 Iterations, No Factor, Danning - Sorting;APV;ADC");
    g_CM_Danning_no_factor[irun]->SetMarkerStyle(8);
    g_CM_Danning_no_factor[irun]->SetMarkerColor(icolor);
    g_CM_Danning_no_factor[irun]->SetLineColor(icolor);
    
    g_CM_Danning[irun] = new TGraphErrors(nadc-1,x_APV,y_CM_Danning[irun],0,y_CM_Danning_err[irun]);
    g_CM_Danning[irun]->SetTitle("3 Iterations, With Factor, Danning - Sorting;APV;ADC");
    g_CM_Danning[irun]->SetMarkerStyle(8);
    g_CM_Danning[irun]->SetMarkerColor(icolor);
    g_CM_Danning[irun]->SetLineColor(icolor); 

    g_CM_Histo[irun] = new TGraphErrors(nadc-1,x_APV,y_CM_Histo[irun],0,y_CM_Histo_err[irun]);
    g_CM_Histo[irun]->SetTitle("Histogramming - Sorting;APV;ADC");
    g_CM_Histo[irun]->SetMarkerStyle(8);
    g_CM_Histo[irun]->SetMarkerColor(icolor);
    g_CM_Histo[irun]->SetLineColor(icolor); 
     

    c3->cd(1);
    if(irun == 0)
      g_CM_Danning_no_factor[irun]->Draw("AP");
    else
      g_CM_Danning_no_factor[irun]->Draw("P");

    c3->cd(2);
    if(irun == 0)
      g_CM_Danning_2iter[irun]->Draw("AP");
    else
      g_CM_Danning_2iter[irun]->Draw("P");


    c3->cd(3);
    if(irun == 0)
      g_CM_Histo[irun]->Draw("AP");
    else
      g_CM_Histo[irun]->Draw("P");
    
    c3->cd(4);
    if(irun == 0)   
      g_CM_Danning[irun]->Draw("AP");
    else
      g_CM_Danning[irun]->Draw("P");

    g_CM_Danning_2iter[irun]->GetYaxis()->SetRangeUser(-50,70);
    g_CM_Danning_no_factor[irun]->GetYaxis()->SetRangeUser(-50,70);
    g_CM_Danning[irun]->GetYaxis()->SetRangeUser(-50,70);
    g_CM_Histo[irun]->GetYaxis()->SetRangeUser(-50,70);
  ...
  c3->Print("../plots/CM_Method_Comparison.pdf)");
