...
void HArm_sim_hist(int Q2 = 368)
{
  ...
  for (Long64_t i=5;i<nevent;i++) {
    ...
    if(G->ev_fnucl == 0 && (*(G->Earm_BBGEM_Track_MID))[0]==0 && (*(G->Earm_BBGEM_Track_NumHits))[0]==5 && G->Earm_BBGEM_Track_ntracks==1 && (*(G->Earm_BBGEM_Track_P))[0]/(G->ev_ep)>0.99){  //used to reproduce the result got by uding det.edum
  ...
  TFile *f = new TFile(output + Form("HArm_sim_rates_%i_GeV.root",Q2),"recreate");
 
  hesum_true->Write("hrate_good_elastics_true");
  hesum_sm->Write("hrate_good_elastics_sm");
}
